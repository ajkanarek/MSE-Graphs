...
class EllinghamDiagram.EllinghamPlot {
    ...
    EllinghamDiagram.EllinghamPlot(PApplet pApplet, int n, int n2, int n3, int n4) {
    ...
    public void ScribePlot() {
        ...
        while (n < this.intersectX.size()) {
            f4 = this.intersectX.get(n).floatValue();
            f = this.intersectY.get(n).floatValue();
            f2 = f4 / 6.469f + (float)this.padding[3] + (float)this.plot.padding[3];
            f3 = (float)(this.padding[0] + this.plot.padding[0]) - f / 2.857f;
            EllinghamDiagram.this.fill(255);
            EllinghamDiagram.this.stroke(0);
            if (f2 + 200.0f > (float)this.applet.width) {
                f2 -= 80.0f;
            }
            EllinghamDiagram.this.rect(f2 + 5.0f, f3 + 5.0f, 70.0f, 35.0f);
            string = this.formatFloat(f4);
            String string2 = this.formatFloat(f);
            EllinghamDiagram.this.fill(0);
            EllinghamDiagram.this.textAlign(37, 101);
            EllinghamDiagram.this.text("T = " + string, f2 + 10.0f, f3 + 10.0f);
            EllinghamDiagram.this.text("\u0394G = " + string2, f2 + 10.0f, f3 + 25.0f);
            ++n;
        }
        n = 0;
        while (n < this.intersectX.size()) {
            ...
            EllinghamDiagram.this.ellipse(f2, f3, 8.0f, 8.0f);
            ++n;
        }
        if (this.labelIndex != -1) {
            float f5 = this.intersectX.get(this.labelIndex).floatValue();
            f4 = this.intersectY.get(this.labelIndex).floatValue();
            f = f5 / 6.469f + (float)this.padding[3] + (float)this.plot.padding[3];
            f2 = (float)(this.padding[0] + this.plot.padding[0]) - f4 / 2.857f;
            EllinghamDiagram.this.fill(255);
            EllinghamDiagram.this.stroke(0);
            if (f + 200.0f > (float)this.applet.width) {
                f -= 80.0f;
            }
            EllinghamDiagram.this.rect(f + 5.0f, f2 + 5.0f, 70.0f, 35.0f);
            String string3 = this.formatFloat(f5);
            string = this.formatFloat(f4);
            EllinghamDiagram.this.fill(0);
            EllinghamDiagram.this.textAlign(37, 101);
            EllinghamDiagram.this.text("T = " + string3, f + 10.0f, f2 + 10.0f);
            EllinghamDiagram.this.text("\u0394G = " + string, f + 10.0f, f2 + 25.0f);
        }
        this.updated = false;
    }
    ...
    public void mouseDragged() {
        ...
        if (this.control == 'O') {
            arrn2 = this.ClosestOnLine(this.OX1, n, this.OX2, this.OY2);
            arrn = arrn2[2] < (arrn3 = this.ClosestOnLine(this.OX2, this.OY2, n2, this.OY3))[2] ? arrn2 : arrn3;
            double d = 0.0;
            this.hOX = arrn[0];
            this.hOY = arrn[1];
            this.controlO = true;
            if (arrn == arrn2) {
                float f2 = 3216.46f;
                int n3 = arrn2[1] - (this.ypos + this.padding[0] + this.plot.padding[0]);
                ...
                float f3 = -1485.71f;
                int n5 = arrn3[0] - (this.xpos + this.padding[3]);
        ...
        if (this.control == 'H') {
            arrn2 = this.ClosestOnLine(n2, this.HY1, this.HX2, this.HY2);
            arrn3 = this.ClosestOnLine(this.HX2, this.HY2, this.HX3, this.HY3);
            arrn = this.ClosestOnLine(this.HX3, this.HY3, n2, this.HY4);
            int[] arrn4 = arrn2[2] < arrn3[2] ? arrn2 : arrn3;
        ...
        if (this.control == 'C') {
            arrn2 = this.ClosestOnLine(n2, this.CY1, this.CX2, this.CY2);
            arrn3 = this.ClosestOnLine(this.CX2, this.CY2, this.CX3, this.CY3);
            arrn = this.ClosestOnLine(this.CX3, this.CY3, n2, this.CY4);
            int[] arrn5 = arrn2[2] < arrn3[2] ? arrn2 : arrn3;
    ...
    public void findIntersects() {
        ...
        while (n < 3) {
            ...
            if (arrbl[n]) {
                ...
                int n3 = 0;
                while (n3 < n2) {
                    Plot.Data data = (Plot.Data)this.plot.data.get(n3);
                    if (!data.hidden) {
                        int n4 = 0;
                        while (n4 < data.Xdata.length - 1) {
                            float[] arrf3 = Geometry.intersection((float)data.Xdata[n4], (float)data.Ydata[n4], (float)data.Xdata[n4 + 1], (float)data.Ydata[n4 + 1], (float)f, (float)f2, (float)f3, (float)f4);
                            if (arrf3 != null) {
    ...
    public boolean getHidden(int n) {
        ...
        return ((Plot.Data)this.plot.data.get((int)n)).hidden;
    }

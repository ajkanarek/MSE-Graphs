...
public class LJSimulation extends PApplet {
...
public void setup(){
 ...
 vsTemp.spos = vsTemp.sposMax*0.95f;
 vsTemp.newspos = vsTemp.sposMax*0.95f;
 ...
 T_Set = new TextLabel(this, str(PApplet.parseInt(L.T)), 360,407);
...
public void draw(){
 ...
 T_Set = new TextLabel(this, str(PApplet.parseInt(L.T*21.8f)) , 360,407);
 T_Set.SetAlign(LEFT,TOP);
 T_Set.draw();
...
public float pow(float base, int exp) {
  ...
  Lattice(float PosX_, float PosY_, float BoxWidth_, float BoxHeight_, int N_, float r0_, float m_, float K_, float initVel_, int Nbins_)
  {
    ...
    TempLine = new float [PApplet.parseInt(BoxWidth*4)];
    WidthIndex = new float [PApplet.parseInt(BoxWidth*4)];
    ...
    while ( n < N){ 
        ...
        float px = (k + (l%2)*0.5f + 3.5f )*( r0*1.01f );
        //if ( px % BoxWidth*0.7 <= r0*1.01 && n>0 ){
          if ( k >= 10 ){
           k = 0;
           l++;
           //px = (k + (l%2)*0.5 + 5 )*( r0*1.01 ) % BoxWidth*0.7;
           px = (k + (l%2)*0.5f +3.5f )*( r0*1.01f ); 
          }     
        float py = (l*0.5f*sqrt(3) + 3.5f)*( r0*1.01f ) ;
...
//     }// end For
  ...
  public void update(){
   ...
   for(int i = 0 ; i < BoxWidth*4 ; i++){
    ...
    TempLine[PApplet.parseInt(BoxWidth*4)-1] = 0;
  ...
  public void Disp()
  {
    ...
    for (int i = 0 ; i<N ; i++){
      ...
      ellipse(Ptemp.pos[0], Ptemp.pos[1], r0*0.9f, r0*0.9f);
    }
    noStroke();
    fill(150);
...
//  }
  ...
  static public void main(String args[]) {
    PApplet.main(new String[] { "--bgcolor=#FFFFFF", "LJSimulation" });
  }
}
